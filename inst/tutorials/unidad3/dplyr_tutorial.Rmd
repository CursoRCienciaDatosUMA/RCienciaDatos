...
`mutate()` siempre devolverá las nuevas variables añadidas a una copia de los datos originales. Si solo quieres devolver las nuevas variables, usa `transmute()`. En el código de más abajo, reemplaza `mutate()` por `transmute()` y observa la diferencia en lo resultados.
...
```{r transmute-check}
"¡Excelente trabajo! `transmute()` y `mutate()` hacen lo mismo, pero `transmute()` solo devuelve las nuevas variables. `mutate()` devuelve una copia de los datos originales con las nuevas variables añadidas al final."
```
